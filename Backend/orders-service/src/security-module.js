...
function registerSecurityModule(app, pool, sendError) {
  const getBcrypt = () => require('bcryptjs');

  // Paso 1: pide la pregunta secreta de un usuario.
  // Siempre responde 200 con una pregunta (genérica si el usuario no existe)
  // para no revelar qué usernames son válidos.
  app.get('/api/security/forgot-password/question', async (req, res) => {
    try {
      ...
      const r = await pool.query('SELECT secret_question FROM users WHERE username=$1', [username]);
  ...
  app.post('/api/security/forgot-password/verify', async (req, res) => {
    try {
      ...
      const r = await pool.query('SELECT username, secret_answer_hash FROM users WHERE username=$1', [username]);
      const user = r.rows[0];
      ...
      const resetToken = jwt.sign({ sub: user.username, purpose: 'password-reset' }, JWT_SECRET, {
