  ...
  return (
    <div className="space-y-4 max-w-sm w-full mx-auto sm:max-w-3xl md:max-w-5xl lg:max-w-7xl xl:max-w-screen-xl px-2">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[0.6875rem] font-bold uppercase tracking-[1.2px] text-[#64748B]">Clientes</p>
          <h1 className="text-xl font-bold text-[#172554]">Gestión de clientes</h1>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) { setFormError(""); setEditCustomer(null); setForm({ name: "", phone: "", address: "", email: "", rut: "", province: "", customerType: segmentType, creditLimit: "" }); setRutStatus(null); setAddressSuggestions([]); } }}>
            <DialogTrigger render={<Button className="flex items-center gap-1.5 h-9 px-3 text-xs font-semibold bg-[#2563EB] hover:bg-[#1D4ED8] text-white"><UserPlus className="h-3.5 w-3.5" />Nuevo cliente</Button>} />
            <DialogContent showCloseButton={false}>
              <DialogHeader>
                <DialogTitle>{editCustomer ? "Editar cliente" : "Nuevo cliente"}</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-3">
                <div className="flex items-center gap-2 rounded border border-[#E2E8F0] bg-[#F8FAFC] px-3 py-2">
                  <span className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">Tipo de cliente</span>
                  <span className="rounded-full bg-[#2563EB]/10 px-2 py-0.5 text-xs font-bold text-[#2563EB]">
                    {CUSTOMER_TYPE_LABELS[segmentType]}
                  </span>
                  <span className="text-[10px] text-[#64748B]">(según modo {mode.toUpperCase()} activo)</span>
                </div>
                <div className="space-y-1">
                  <label htmlFor="customers-page-f178" className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">Nombre *</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">RUT{form.customerType === "company" ? " *" : " (opcional)"}</label>
                    ...
                    {rutStatus && (
                      <p className={cn("flex items-center gap-1 text-[10px] font-semibold", rutStatus.valid ? "text-[#0D9488]" : "text-red-500")}>
                  ...
                  <div className="space-y-1">
                    <label htmlFor="customers-page-f193" className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">Teléfono</label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label htmlFor="customers-page-f199" className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">Email</label>
                  ...
                  <div className="relative space-y-1">
                    <label htmlFor="customers-page-f203" className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">Dirección</label>
                    ...
                    {addressFocused && (addressLoading || addressSuggestions.length > 0) && (
                      <div className="absolute z-10 mt-1 w-full max-h-48 overflow-y-auto rounded border border-[#E2E8F0] bg-white shadow-lg">
                        {addressLoading && (
                          <p className="px-3 py-2 text-xs text-[#64748B]">Buscando direcciones...</p>
                        )}
                        {!addressLoading && addressSuggestions.map((s, i) => (
                          <button
                            ...
                            className="flex w-full items-start gap-1.5 border-b border-[#F8FAFC] px-3 py-2 text-left text-xs text-[#172554] last:border-b-0 hover:bg-[#F8FAFC]"
                          >
                            <MapPin className="mt-0.5 h-3 w-3 shrink-0 text-[#2563EB]" />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <label htmlFor="customers-page-f235" className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">Provincia / Región</label>
                  ...
                  <div className="space-y-1">
                    <label htmlFor="customers-page-f239" className="text-[10px] font-bold uppercase tracking-[0.92px] text-[#64748B]">Límite de crédito (fiado)</label>
                ...
                <div className="flex justify-end gap-2 pt-1">
                  ...
                  <Button type="submit" size="sm" className="bg-[#2563EB] hover:bg-[#1D4ED8] text-white" disabled={creating}>{creating ? "Guardando..." : "Guardar"}</Button>
                </div>
              </form>
            </DialogContent>
          </Dialog>
          )}
          <span className="text-xs text-[#64748B]">{filtered.length} clientes</span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        ...
        <div
          className={cn(
            ...
            segmentType === "individual" ? "border-[#0D9488]/30 bg-[#0D9488]/10 text-[#0D9488]" : "border-[#2563EB]/30 bg-[#2563EB]/10 text-[#2563EB]"
          )}
      ...
      <div className="space-y-2">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <p className="text-sm font-bold text-[#172554]">{customer.name}</p>
                  <span className={cn(
                    "rounded-full px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide",
                    customer.customerType === "individual" ? "bg-[#0D9488]/10 text-[#0D9488]" : "bg-[#2563EB]/10 text-[#2563EB]"
                  )}>
                ...
                <div className="flex flex-wrap gap-x-3 gap-y-0.5 text-xs text-[#64748B] mt-0.5">
                  {customer.rut && <span className="font-mono font-semibold text-[#172554]">RUT {customer.rut}</span>}
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button type="button" onClick={() => openEdit(customer)} className="inline-flex items-center justify-center rounded-lg border border-border min-h-[36px] min-w-[36px] text-[#2563EB] hover:bg-[#2563EB]/5 active:scale-[0.95] transition-colors" title="Editar">
        ...
        {filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center rounded border border-[#E2E8F0] bg-white py-16">
            <User className="h-10 w-10 text-[#E2E8F0]" />
            <p className="mt-3 text-sm font-medium text-[#64748B]">Sin clientes</p>
            <p className="mt-1 text-xs text-[#64748B]/70">Agrega tu primer cliente para comenzar.</p>
          </div>
        )}
      </div>
    </div>
